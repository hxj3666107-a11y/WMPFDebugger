...
self.location = { href: "file:///tmp/wxisaac_runtime_test.js" };

const fs = require("fs");

const SEED = process.argv[2];
if (!SEED) {
  console.error("Usage: node tools/wxisaac_keygen.js <seed> [output.bin]");
  process.exit(2);
}
const OUT_FILE = process.argv[3] || `/tmp/wx_key_${SEED}.bin`;

let captured = null;

var Module = {
  wasmBinary: fs.readFileSync(
    "/home/huang-justin/WMPFDebugger-linux/wasm_video_decode.wasm"
  ),

  noInitialRun: true,

  wasm_isaac_generate: function(ptr, len) {
    try {
      captured = Uint8Array.from(
  ...
  onRuntimeInitialized: function() {
    ...
    console.log("WxIsaac64 TYPE:", typeof Module.WxIsaac64);

    if (typeof Module.WxIsaac64 !== "function") {
      console.log(
        "MATCHING MODULE KEYS:",
        Object.keys(Module).filter(k => /isaac/i.test(k))
      );
      process.exit(2);
    }
    ...
    console.log(
      "PROTOTYPE:",
      Object.getOwnPropertyNames(Module.WxIsaac64.prototype)
    );

    try {
      const x = new Module.WxIsaac64(SEED);
      ...
      if (captured) {
        const reversed = Uint8Array.from(captured).reverse();

        console.log(
          "RAW_HEAD:",
          Buffer.from(captured.slice(0, 32)).toString("hex")
        );

        fs.writeFileSync(
          OUT_FILE,
          Buffer.from(reversed)
        );
        console.log("KEY_FILE:", OUT_FILE);

        console.log(
          "KEY_HEAD:",
          Buffer.from(reversed.slice(0, 32)).toString("hex")
        );
      }

    } catch (e) {
      console.error("GENERATE_ERROR:", e.stack || e);
    }
...
var wasm_isaac_generate = Module.wasm_isaac_generate;
var VTS_WASM_URL = "";
